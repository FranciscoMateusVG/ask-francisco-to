    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <div className="bg-gradient-to-r from-purple-900 via-purple-800 to-indigo-950 px-6 py-8">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-purple-300 text-sm">
            ...
            {completed.length > 0 && <span> · {completed.length} completed</span>}
          </p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-6 -mt-4">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 px-5 py-4 flex items-center gap-3 mb-6">
          ...
          {unlocked && (
            <span className="text-green-500 font-semibold text-sm">✓ unlocked</span>
          )}
          {unlockError && (
            <span className="text-red-400 text-sm font-medium">Wrong secret</span>
          )}
          {!unlocked && !unlockError && (
            <span className="text-gray-400 text-xs ml-auto hidden sm:block" aria-hidden="true">
              Enter the secret word to manage requests
            </span>
          )}
        </div>
      ...
      <div className="flex justify-between items-start gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-gray-400">
            ...
            <span className="font-medium text-gray-500">{t.name || 'Anonymous'}</span>
            {t.contact && (
              <>
                <span>·</span>
                <span>{t.contact}</span>
          ...
          {expanded && (
            <div className="mt-3 space-y-2 text-xs text-gray-600 bg-gray-50 rounded-lg p-3">
              {t.steps_to_reproduce && (
                <div>
                  <span className="font-semibold text-gray-500 uppercase tracking-wide">Steps: </span>
                  <span className="whitespace-pre-wrap">{t.steps_to_reproduce}</span>
                </div>
              )}
              {t.expected_behavior && (
                <div>
                  <span className="font-semibold text-gray-500 uppercase tracking-wide">Expected: </span>
                  <span>{t.expected_behavior}</span>
                </div>
              )}
              {t.actual_behavior && (
                <div>
                  <span className="font-semibold text-gray-500 uppercase tracking-wide">Actual: </span>
